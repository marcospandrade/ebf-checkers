...
public class CheckersTrainer extends JFrame implements WindowListener,
		ComponentListener, CheckersGameListener, ListSelectionListener {

	private class CheckersPlayerTable extends JPanel implements KeyListener,
		...
		public CheckersPlayerTable() {
					...
					if (table.getRowCount() > 0) {
						for (int i = 0; i < selectedRows.length; i++)
							if (selectedRows[i] < model.getRowCount())
								table.addRowSelectionInterval(selectedRows[i],
										selectedRows[i]);
							else
								table.addRowSelectionInterval(model
										.getRowCount() - 1,
										model.getRowCount() - 1);
					}
				}
			...
			up = new AbstractAction() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					for (int i = 0; i < selectedRows.length; i++) {
						model.moveRow(selectedRows[i], selectedRows[i],
								--selectedRows[i]);
					}
			...
			randomize = new AbstractAction() {
				@Override
				public void actionPerformed(ActionEvent e) {
					// Get Selected Values for later
					int[] selectedRows = table.getSelectedRows();
					Object[] selectedValues = new Object[selectedRows.length];
					for (int i = 0; i < selectedValues.length; i++)
						selectedValues[i] = model
								.getValueAt(selectedRows[i], 0);

					// Move each row to a random index
					for (int i = 0; i < model.getRowCount(); i++) {
						model.moveRow(i, i, (int) (Math.random() * model
								.getRowCount()));
					}

					// Reselect all things that were selected before
			...
			add
					.putValue(Action.SHORT_DESCRIPTION,
							"Adds the selected Players from the Player List to this group table");
			insert
					.putValue(
							Action.SHORT_DESCRIPTION,
							"Inserts the selected Players from the Player List to this group table at the selected row");
			delete.putValue(Action.SHORT_DESCRIPTION,
					"Deletes the selected values in this table");
			up.putValue(Action.SHORT_DESCRIPTION,
					"Moves all selected values in this table one row up");
			down.putValue(Action.SHORT_DESCRIPTION,
					"Moves all selected values in this table one row down");
			clear.putValue(Action.SHORT_DESCRIPTION,
					"Clears all players from this table");
			randomize.putValue(Action.SHORT_DESCRIPTION,
					"Randomizes the order of the players in this table");
				...
				protected JTableHeader createDefaultTableHeader() {
					return new JTableHeader(columnModel) {
						...
						public String getToolTipText(MouseEvent e) {
							int index = columnModel.getColumnIndexAtX(e
									.getPoint().x);
			...
			table.setTransferHandler(new TransferHandler() {
				...
				public boolean canImport(TransferHandler.TransferSupport info) {
					if (!info
							.isDataFlavorSupported(PlayerData.playerDataFlavor))
						return false;
				...
				public boolean importData(TransferHandler.TransferSupport info) {
					...
					// Check for playerDataFlavor
					if (!info
							.isDataFlavorSupported(PlayerData.playerDataFlavor))
						return false;

					if (info.getSourceDropActions() != COPY
							&& info.getSourceDropActions() != MOVE)
						return false;

					try {
						Object data = (info.getTransferable())
								.getTransferData(PlayerData.playerDataFlavor);
						Object source = (info.getTransferable())
								.getTransferData(PlayerData.sourceFlavor);

						int index = table.rowAtPoint(info.getDropLocation()
								.getDropPoint());
						if (index == -1)
						...
						if (info.getSourceDropActions() == COPY) {
							...
							table.setRowSelectionInterval(initRowIndex,
									index - 1);
						} else if (source == table) {
							int[] selectedRows = (int[]) data;

							// Move everything to the bottom first
							for (int i = 0; i < selectedRows.length; i++) {
								int moveIndex = selectedRows[i] - i;
								model.moveRow(moveIndex, moveIndex, model
										.getRowCount() - 1);
							}

							// Move everything to index if needed and select the
							// moved stuff
							...
							for (int i = indices.length - 1; i >= 0; i--)
								sourceModel.removeRow(indices[i]);
						}
		...
		public void updateStatus() {
			...
			insert.setEnabled(enabled && listHasSelection
			...
			if (enabled)
				enabled = table.getSelectedRowCount() > 0;
			delete.setEnabled(enabled);

			if (enabled) {
				int[] indices = table.getSelectedRows();
				up.setEnabled(indices[0] > 0);
				down.setEnabled(indices[indices.length - 1] < model
						.getRowCount() - 1);
			} else {
				up.setEnabled(false);
	...
	private static class PlayerData implements Transferable {
		...
		static {
			try {
				playerDataFlavor = new DataFlavor(
						DataFlavor.javaJVMLocalObjectMimeType);
				sourceFlavor = new DataFlavor(
						DataFlavor.javaJVMLocalObjectMimeType);
	...
	private static final String[] COLUMN_TOOL_TIPS = new String[] {
			...
			"The number of games the CheckersPlayer has drawn",
			"The number of games the CheckersPlayer has lost",
			"The number of games the CheckersPlayer has played" };
	...
	private static final float WINS_X_RATIO = 0.2f, DRAWS_X_RATIO = 0.45f,
			LOSSES_X_RATIO = 0.7f;
	...
	public CheckersTrainer(CheckersGameManager gameManager) {
		...
		playersList.setTransferHandler(new TransferHandler() {
			...
			public boolean canImport(TransferHandler.TransferSupport info) {
				try {
					return info.getTransferable().getTransferData(
							PlayerData.sourceFlavor) instanceof JTable;
				} catch (Exception e) {
					e.printStackTrace();
					return false;
				}
			...
			public boolean importData(TransferHandler.TransferSupport info) {
				...
				try {
					int[] indices = (int[]) info.getTransferable()
							.getTransferData(PlayerData.playerDataFlavor);
		...
		group1Table
				.setToolTipText("A table of the players in Group 1 and their statistics");
		border = BorderFactory.createTitledBorder(BORDER, "Group 1");
		group1Table.setBorder(border);

		group2Table = new CheckersPlayerTable();
		group2Table
				.setToolTipText("A table of the players in Group 2 and their statistics");
		border = BorderFactory.createTitledBorder(BORDER, "Group 2");
		group2Table.setBorder(border);

		playerGroupsSplitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT,
				group1Table, group2Table);
		...
		splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, playersSP,
				playerGroupsSplitPane);
		...
		graphSplitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, graphSP,
				graph);
		...
		for (int i = 0; i < CheckersPlayerLoader.getNumCheckersPlayers(); i++) {
			...
			if (player.equals(Human.class.getSimpleName()))
				continue;
		...
		TableColumnModel traineesColumnModel = group1Table.table
				.getColumnModel();
		TableColumnModel opponentsColumnModel = group2Table.table
				.getColumnModel();
	...
	public void gameEnded(CheckersGameEvent ce) {
		...
		if (outcome != CheckersGameManager.INTERRUPTED
				&& outcome != CheckersGameManager.GAME_IN_PROGRESS) {

			int group1Column = 2;
			int group2Column = 2;
			if (outcome != CheckersGameManager.DRAW) {
				if (outcome == CheckersGameManager.PLAYER1_WINS
						^ !group1IsPlayer1) {
					group1Column = 1;
					group2Column = 3;
				} else {
			...
			group1Table.model.setValueAt((Integer) group1Table.model
					.getValueAt(group1Row, group1Column) + 1, group1Row,
					group1Column);
			int traineeGames = (Integer) group1Table.model.getValueAt(
					group1Row, 4) + 1;
			group1Table.model.setValueAt(traineeGames, group1Row, 4);
			group2Table.model.setValueAt((Integer) group2Table.model
					.getValueAt(group2Row, group2Column) + 1, group2Row,
					group2Column);
			group2Table.model.setValueAt((Integer) group2Table.model
					.getValueAt(group2Row, 4) + 1, group2Row, 4);

			try {
				int gameCount = (Integer) group1Table.model.getValueAt(
						group1Row, 4);
				wins.get(group1Row).append(
						new double[] {
								gameCount,
								100.0
			...
			if (group1Row == traineeStartIndex) {
				...
				if (group2Row == opponentStartIndex)
					group1IsPlayer1 = !group1IsPlayer1;
			}
	...
	public void initActions() {
		...
		resetData = new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int c = 1; c < group1Table.table.getColumnCount(); c++)
					group1Table.model.setValueAt(0, 0, c);

				for (int r = 0; r < group2Table.model.getRowCount(); r++) {
					for (int c = 1; c < group2Table.model.getColumnCount(); c++)
						group2Table.model.setValueAt(0, r, c);
				}
		...
		start = new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				if (((JToggleButton) e.getSource()).isSelected()) {
					...
					start.putValue(Action.SHORT_DESCRIPTION,
							"Stops the training");
		...
		checkersGUI.putValue(Action.SHORT_DESCRIPTION,
				"Disable the CheckersGUI");
		resetData.putValue(Action.SHORT_DESCRIPTION,
				"Resets the statistics for all players");
	...
	private void initTraining() {
		try {
			...
			for (int i = 0; i < numTrainees; i++) {
				...
				for (int j = 0; j < CheckersPlayerLoader
						.getNumCheckersPlayers(); j++) {
					if (name.equals(CheckersPlayerLoader
							.getPlayerDisplayName(j))) {
						player = CheckersPlayerLoader.getPlayerClass(j)
								.newInstance();
			...
			opponentStartIndex = Math
					.max(0, group2Table.table.getSelectedRow());
			gameManager.setPlayer1(group1[traineeStartIndex]);
			gameManager.setPlayer2(group2[opponentStartIndex]);
			group1Table.table.setRowSelectionInterval(traineeStartIndex,
					traineeStartIndex);
			group2Table.table.setRowSelectionInterval(opponentStartIndex,
					opponentStartIndex);
			gameManager.newGame();
			System.gc();
		} catch (Exception e) {
			e.printStackTrace();
			JOptionPane.showMessageDialog(CheckersTrainer.this, e.getMessage(),
					"Error Creating Players", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	private void updateTrainerStatus() {
		start.setEnabled(group1Table.model.getRowCount() > 0
				&& group2Table.model.getRowCount() > 0);
	}

	@Override
	public void valueChanged(ListSelectionEvent e) {
		if (e.getSource() == playersList)
			updatePlayersList();
		else if (e.getSource() == graphPlayersList) {
			if (graphPlayersModel.size() > 0) {
				int selected = graphPlayersList.getSelectedIndex();
				if (selected == -1)
					graphPlayersList.setSelectedIndex(oldGraphPlayersIndex);
				else {
					DataSet data = wins.get(oldGraphPlayersIndex);
					axisX.detachDataSet(data);
					axisY.detachDataSet(data);
